        ...
        <div className="grid items-center gap-12 lg:grid-cols-2 lg:gap-16">
          <Reveal className="order-2 lg:order-1">
            <div className="relative">
              ...
              <div className="relative overflow-hidden rounded-3xl shadow-2xl shadow-ink-900/15">
                <img src={SIGNBOARD_PHOTO} alt="CAC Registrations Port Harcourt office signboard on Ada-George Road, Rivers State" className="h-[380px] w-full object-cover sm:h-[460px]" loading="lazy" width="1000" height="460" />
              </div>
              <div className="absolute -bottom-6 left-6 z-10 flex items-center gap-3 rounded-2xl border border-ink-100 bg-white p-4 shadow-xl dark:border-ink-800 dark:bg-ink-900 sm:left-10">
                ...
                <div>
                  <p className="font-display text-sm font-bold text-ink-900 dark:text-white">Led by {BUSINESS.owner}</p>
          ...
          <div className="order-1 lg:order-2">
            <SectionHeading
              ...
              subtitle={`${BUSINESS.name} is a professional CAC registration service led by ${BUSINESS.owner}. We help entrepreneurs, clubs, associations, and unions get registered quickly, correctly, and affordably.`}
            />
          ...
          {STATS.map((stat, i) => (
            <Reveal key={stat.label} delay={i * 90}>
              <div className="card group p-6 text-center hover:-translate-y-1 hover:shadow-lg hover:shadow-brand-700/10">
                <p className="font-display text-3xl font-extrabold text-brand-700 sm:text-4xl dark:text-brand-400">{stat.value}</p>
                <p className="mt-1.5 text-sm font-medium text-ink-500 dark:text-ink-400">{stat.label}</p>
              </div>
            </Reveal>
          ))}
        </div>
